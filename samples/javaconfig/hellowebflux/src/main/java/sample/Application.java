...
public class Application implements WebFluxConfigurer {
	...
	private Mono<Boolean> currentUserMatchesPath(Mono<Authentication> authentication, AuthorizationContext context) {
		return authentication.flatMap( a -> Mono.just(context.getVariables().get("user").equals(a.getName())));
	}
